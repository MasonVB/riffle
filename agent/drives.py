...
SCHEMA = {
    ...
    "flag": (["target_type", "target_id", "reason"], [],
             lambda p: {"target_type": _enum(p["target_type"], ("post", "comment")),
                        "target_id": _i(p["target_id"], "target_id"),
                        "reason": _trim(p["reason"], 1, 200, "reason")}),
    "seal": (["hash", "label"], [],
    ...
    "attestation": (["subject", "claim"], ["cls", "evidence"],
                    lambda p: {"cls": _s(p.get("cls") or "observation", 1, 40, "cls"),
                               ...
                               "claim": _trim(p["claim"], 1, 1000, "claim"),
    ...
    "sign": (["kind"], ["row", "expiry", "hash", "label", "subject", "claim"],
             lambda p: {"kind": _enum(p["kind"], ("payout", "seal", "attest")),
                        ...
                        "claim": _trim(p.get("claim") or "", 0, 1000, "claim")}),
    "ask_operator": (["question"], ["why"],
                     lambda p: {"question": _s(p["question"], 8, 600, "question"),
                                "why": _trim(p.get("why") or "", 0, 400, "why")}),
    "read_page": (["url"], ["why"],
                  lambda p: {"url": _s(p["url"], 12, 500, "url"),
                             "why": _trim(p.get("why") or "", 0, 300, "why")}),
    "library_put": (["title", "body"], ["kind", "tags", "summary", "source"],
                    lambda p: {"title": _s(p["title"], 3, 200, "title"),
                               ...
                               "summary": _trim(p.get("summary") or "", 0, 600, "summary"),
    ...
    "build": (["entry", "files"], ["note"],
              lambda p: {"entry": _s(p["entry"], 3, 44, "entry"),
                         ...
                         "note": _trim(p.get("note") or "", 0, 400, "note")}),
    "fetch": (["what"], [],
    ...
    "noop": ([], ["why"], lambda p: {"why": _trim(p.get("why", "nothing worth doing"), 0, 1200, "why")}),
    ...
    "request_cycle": (["reason"], [],
                      lambda p: {"reason": _trim(p["reason"], 20, 400, "reason")}),
    "read_thread": (["post_id"], [],
                    lambda p: {"post_id": _i(p["post_id"], "post_id")}),
    ...
    "close_project": (["reason"], [],
                      lambda p: {"reason": _trim(p["reason"], 20, 600, "reason")}),
